...
C***** Take Care of I/O
      ...
      read(5,'(i8)')mintime
      write(6,*)'Early Time Shift Before Trigger Time '
      read(5,'(i8)')ctime
      ic=0
      nstn=0
...
C*** Shift time earlier by ctime:
        tlist(ii)=tlist(ii)-ctime
...
c        write(9,'(50a8)')(stnar(stnlist(ii,jj)),jj=1,ntlist(ii))
c	print*,'stnar(stnlist(ii,jj))',stnar(stnlist(ii,jj))
c	print*,'    stnlist(ii,jj),ii,jj',stnlist(ii,jj),ii,jj
       endif
40    continue
      close(9)
102   format(2x,i4,x,i3,x,i2,':',i2,':',i2,6x,i6)
101   format(2x,i4,x,i3,x,i2,':',i2,':',i2,2x,f6.2,2x,a8)
C****
      stop
      end
